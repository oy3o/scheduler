...
## 2026-05-15 - Early Return for Interpolation Operations
**Learning:** When using linear interpolation for metrics like percentiles (e.g., `vLo + frac*(vHi-vLo)`), computing the upper bound (`vHi`) using O(N) operations is completely wasted if the fractional component is exactly zero.
**Action:** Always check if the fractional weight is zero before performing expensive adjacent-element lookups or array scans, and return the lower bound early.
