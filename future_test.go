...
func TestFuture_PanicIsolation(t *testing.T) {
	...
	f, err := SubmitFunc(g, 10, func(c Context) (int, error) {
		panic("bloody sincerity\r\nthis should be hidden\tgoroutine debug.Stack")
	})
	if err != nil {
		t.Fatalf("SubmitFunc failed: %v", err)
	}
	...
	if strings.Contains(getErr.Error(), "this should be hidden") {
		t.Errorf("Expected error to truncate after newline/carriage return, got: %v", getErr)
	}
