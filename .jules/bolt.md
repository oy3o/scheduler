...
## 2025-05-25 - Percentile Early Return
**Learning:** When calculating percentiles via linear interpolation between indices, if the calculated fractional component (`frac`) is exactly zero (meaning the percentile lands perfectly on an integer index), calculating the "next" upper bound (`vHi`) via an O(N) scan (`slices.Min(data[lo+1:])`) is unnecessary wasted work.
**Action:** Always include an early return (`if frac == 0`) before performing the upper bound calculation to skip the O(N) lookup.
