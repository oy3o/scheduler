...
func sortedPercentileMut(data []float64, p float64) float64 {
	...
	frac := idx - float64(lo)
	if frac == 0 {
		// ⚡ Bolt: Skipped O(N) adjacent-element lookup.
		// When calculating exact percentiles (frac == 0), finding vHi via slices.Min
		// is unnecessary. Returning early saves significant CPU cycles in the monitoring loop.
		return vLo
	}
