...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// 🛡️ Sentinel: Prevent stack trace leakage and log spoofing.
			// Truncating at alternate vertical whitespace (\r, \f, \v) prevents both
			// stack trace leakage and terminal overwrite/log-spoofing attacks where
			// carriage returns can maliciously alter console output.
