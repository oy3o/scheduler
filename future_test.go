...
func TestFuture_PanicIsolation_Spoofing(t *testing.T) {
	g := New(DefaultConfig())
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	// Submit a task that panics with carriage returns
	f, err := SubmitFunc(g, 10, func(c Context) (int, error) {
		panic("error message\r\nfake stack trace")
	})
	if err != nil {
	...
	if strings.Contains(getErr.Error(), "fake stack trace") {
		t.Errorf("Expected sanitized error without fake stack trace, got: %v", getErr)
	}
}
