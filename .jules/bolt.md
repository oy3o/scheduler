...
## 2024-06-28 - Fast Path for Exact Percentiles
**Learning:** When calculating percentiles with linear interpolation on unsorted slices, finding the upper bound for the fractional component (e.g., via `slices.Min`) is an O(N) operation. This overhead is incurred even when the percentile maps to an exact integer index where the fractional part is zero.
**Action:** Always compute the fractional index difference (`frac`) first and return early if `frac == 0` to skip the unnecessary O(N) lookup for exact percentiles.
