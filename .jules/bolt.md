...
## 2025-05-18 - Early Return on Exact Percentiles
**Learning:** In percentile calculations that use linear interpolation on unsorted slices, finding the upper bound for the fractional component (e.g., via `slices.Min`) is an O(N) operation.
**Action:** Compute the fractional index difference (`frac`) first and return early if `frac == 0` to skip the unnecessary O(N) lookup for exact percentiles, improving performance.
