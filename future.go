...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// and log spoofing. If the user's panic payload contains multiple lines
			// or carriage returns, we strip everything after the first line to keep
			// the Future API clean and prevent terminal overwrite attacks.
