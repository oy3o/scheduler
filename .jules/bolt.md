...
## 2024-05-25 - Early Return in Percentile Interpolation
**Learning:** When calculating values via linear interpolation (like percentiles), implementing early returns to skip expensive adjacent-element lookups (e.g., O(N) slice scans) if the fractional weight is exactly zero significantly reduces CPU overhead, especially when exact indices are common.
**Action:** Always check if the fractional component of an interpolation calculation is zero and return early to avoid unnecessary computations or data access.
