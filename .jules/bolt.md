...
## 2025-05-25 - Skip O(N) Sub-Slice Scan for Exact Percentiles
**Learning:** When calculating percentiles with linear interpolation on unsorted slices, finding the fractional component often requires scanning the remaining sub-slice (`slices.Min(data[lo+1:])`) which is an O(N) operation.
**Action:** Always compute the fractional index difference (`frac = idx - float64(lo)`) *before* searching for the upper bound element. If `frac == 0`, return early to avoid the unnecessary O(N) lookup.
