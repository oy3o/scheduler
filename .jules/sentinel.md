...
## 2024-06-23 - [Fix Log Spoofing via Alternate Vertical Whitespace]
**Vulnerability:** The panic payload sanitization in `Future` only truncated at `\n`, allowing `\r`, `\f`, or `\v` to be included in the public error message, enabling log spoofing or terminal output manipulation.
**Learning:** Using `strings.Index` for just `\n` is insufficient to sanitize multiline input against terminal escape sequences or alternate line breaks that can manipulate log viewers and console outputs.
**Prevention:** Always use `strings.IndexAny` with all vertical whitespace characters (`\n\r\f\v`) when sanitizing single-line log/error messages from untrusted input or panic payloads.
