...
## 2025-02-14 - Early Return in Linear Interpolation
**Learning:** When calculating values via linear interpolation (like percentiles), if the fractional index is exactly zero, computing the adjacent element (e.g., via slices.Min) incurs an unnecessary O(N) scan.
**Action:** Implement early returns for zero-fraction cases before executing expensive adjacent-element lookups.
