...
func sortedPercentileMut(data []float64, p float64) float64 {
	...
	frac := idx - float64(lo)
	// ⚡ Bolt: Early return for exact percentiles (frac == 0).
	// Finding the next element (vHi) requires scanning the remaining unsorted
	// partition with slices.Min, which is an O(N) operation.
	// Skipping it when frac == 0 eliminates this overhead completely.
	if frac == 0 {
		return vLo
	}
