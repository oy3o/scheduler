...
## 2025-05-20 - Early Return on Exact Percentiles in QuickSelect
**Learning:** When calculating percentiles with linear interpolation on unsorted slices using QuickSelect, finding the upper bound for the fractional component (e.g., via `slices.Min`) requires an O(N) scan.
**Action:** Compute the fractional index difference (`frac`) first and immediately return the lower bound if `frac == 0` to skip the unnecessary O(N) adjacent-element lookup.
