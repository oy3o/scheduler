...
## 2024-05-25 - Early Return in Percentile Calculation
**Learning:** When calculating percentiles with linear interpolation on unsorted slices, finding the upper bound for the fractional component (via `slices.Min`) is an O(N) operation. We don't always need this value if the percentile lands exactly on an integer index (i.e. fraction is 0).
**Action:** Compute the fractional index difference (`frac`) first and return early if `frac == 0` to skip the unnecessary O(N) lookup for exact percentiles, which dramatically improves performance for those exact cases.
