...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// and log spoofing. If the user's panic payload itself contains multiple
			// lines or vertical whitespace (e.g., \r, \n, \v, \f), we strip everything
			// after the first line to keep the Future API clean and secure.
