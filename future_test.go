...
func TestFuture_PanicIsolation_AlternateWhitespace(t *testing.T) {
	...
	f, err := SubmitFunc(g, 10, func(c Context) (int, error) {
		panic("sneaky payload\rdebug.Stack() output here")
	})
	if err != nil {
		t.Fatalf("SubmitFunc failed: %v", err)
	}
	...
	if strings.Contains(getErr.Error(), "debug.Stack") {
		t.Errorf("Expected sanitized error, got: %v", getErr)
	}
	if !strings.Contains(getErr.Error(), "sneaky payload") {
		t.Errorf("Expected base payload, got: %v", getErr)
	}
}
