...
**Learning:** When calculating values via linear interpolation (like percentiles), computing the exact fractional offset allows for early returns. If the fractional weight is exactly zero, the expensive adjacent-element lookups (like an O(N) slice scan to find the minimum of the remaining elements) can be entirely skipped.
**Action:** Implement early returns based on fractional weights before triggering expensive O(N) adjacent-element queries during percentile interpolation.
