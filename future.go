...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// 🛡️ Sentinel: Sanitize the public error to prevent stack trace leakage and log spoofing.
			// If the user's panic payload itself contains multiple lines or alternate vertical whitespace
			// (e.g. they re-panicked an error with a stack trace), we strip everything after
			// the first line break to keep the Future API clean and prevent terminal overwrite.
