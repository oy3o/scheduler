...
## 2025-06-25 - Skip O(N) Lookup for Exact Percentiles
**Learning:** When calculating percentiles with linear interpolation on unsorted slices (like in `monitor.go`), finding the upper bound for the fractional component (e.g., via `slices.Min`) is an O(N) operation.
