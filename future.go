...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// and log spoofing/terminal overwrites via alternate vertical whitespace.
			// If the user's panic payload contains multiple lines (or carriage returns),
			// we strip everything after the first line to keep the Future API clean.
