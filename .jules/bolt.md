...
## 2025-05-18 - Early Return for Exact Percentiles
**Learning:** When computing percentiles with fractional interpolation, exact indexes (where the fractional part is zero) needlessly scan the remainder of the slice to find the next element using `slices.Min()`. In tight loops or large slices, this O(N) scan causes a measurable slowdown.
**Action:** When performing linear interpolation between indices, always implement an early return if the fractional weight is exactly zero to skip unnecessary adjacent-element lookups.
