...
## 2025-05-15 - Skipping O(N) Upper-Bound Lookup for Exact Percentiles
**Learning:** When calculating percentiles with linear interpolation on unsorted slices, finding the upper bound for the fractional component (e.g., via `slices.Min` or `slices.Max`) is an O(N) operation.
