...
## 2025-10-24 - Skip O(N) lookup on exact percentile bounds
**Learning:** When calculating values via linear interpolation (like percentiles), computing the interpolation weights can sometimes result in exactly zero for the fractional component. Continuing the calculation forces expensive adjacent-element lookups (e.g., O(N) slice scans) that are mathematically multiplied by zero and discarded anyway.
**Action:** Always implement early returns to skip expensive adjacent-element lookups if the fractional weight is exactly zero during interpolation.
