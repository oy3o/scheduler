...
## 2025-05-14 - Skip Redundant Scans During Interpolation
**Learning:** When calculating values via linear interpolation (like percentiles), computing an exact fractional index often results in a fractional weight of exactly zero. Without an early return, the algorithm may still perform an expensive O(N) adjacent-element scan (`slices.Min` or similar) just to multiply the result by 0 and discard it.
**Action:** Always implement an early return (e.g., `if frac == 0 { return vLo }`) before invoking expensive operations in linear interpolation paths to skip unnecessary slice scans.
