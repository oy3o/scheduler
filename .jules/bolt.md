...
## 2025-05-18 - Early Return on Exact Percentiles in O(N) QuickSelect
**Learning:** When calculating percentiles with linear interpolation on unsorted slices (e.g., `sortedPercentileMut`), finding the upper bound for the fractional component via `slices.Min` is an O(N) operation.
**Action:** Always compute the fractional index difference (`frac`) first and return early if `frac == 0` to skip the unnecessary O(N) lookup for exact percentiles, cutting the execution time in half for exact-index cases.
