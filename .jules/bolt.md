...
## $(date +%Y-%m-%d) - [Early Return on Exact Percentile Match]
**Learning:** In the `sortedPercentileMut` latency monitor loop, when calculating the $p$-th percentile via linear interpolation, the fractional weight (`frac`) is often exactly zero when $p$ perfectly lands on an exact index boundary. Continuing to calculate the upper bound (`vHi`) using `slices.Min` across the remaining adjacent slice elements forces an unnecessary $O(N)$ scan, wasting CPU cycles on a value that will immediately be multiplied by zero.
**Action:** Always check `frac == 0` when linearly interpolating between two dynamically searched values in an array/slice. Add an early return if `frac` is zero to skip the expensive lookup of the second value.
