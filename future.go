...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// 🛡️ Sentinel: Prevent stack trace leakage and log spoofing via alternate vertical whitespace.
			// If the user's panic payload itself contains multiple lines or spoofing characters
			// (e.g. they re-panicked an error with a stack trace or malicious carriage returns),
			// we strip everything after the first line-breaking character (\n, \r, \f, \v) to keep the Future API clean.
