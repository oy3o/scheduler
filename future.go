...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// re-panicked an error with a stack trace) or carriage returns, we strip
			// everything after the first alternate vertical whitespace to keep the
			// Future API clean and prevent terminal overwrites.
