...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// If the user's panic payload itself contains multiple lines using \n, \r, \f, or \v
			// (e.g., injected carriage returns or stack traces), we strip everything after
			// the first occurrence to keep the Future API clean.
