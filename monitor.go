...
func sortedPercentileMut(data []float64, p float64) float64 {
	...
	frac := idx - float64(lo)
	// ⚡ Bolt Optimization: Early return when fraction is exactly zero.
	// Skips the expensive O(N) adjacent-element scan (slices.Min) when the
	// percentile index lands perfectly on an integer boundary.
	if frac == 0 {
		return vLo
	}
