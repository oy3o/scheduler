...
## 2025-05-20 - Skipping O(N) Interpolation Scans in Unsorted Percentiles
**Learning:** When calculating percentiles with linear interpolation on unsorted slices (like in `sortedPercentileMut`), finding the upper bound for the fractional component via `slices.Min` is an O(N) operation.
