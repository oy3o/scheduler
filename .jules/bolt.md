...
## 2024-06-14 - Early Returns for Exact Percentiles to skip O(N) overhead
**Learning:** When calculating percentiles with linear interpolation on unsorted slices, finding the upper bound for the fractional component (e.g., via `slices.Min`) is an O(N) operation over remaining elements. For exact percentiles, this is a waste of CPU.
