...
## 2026-05-21 - [Optimize Percentile Interpolation]
**Learning:** When calculating percentiles via linear interpolation, an early return can skip expensive adjacent-element lookups (like O(N) slice scans) if the fractional weight is exactly zero.
**Action:** Always check if interpolation fractions are zero before performing expensive calculations to find the next element, particularly in hot monitoring loops.
