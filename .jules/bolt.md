...
## 2025-05-15 - Early Return in Linear Interpolation Percentiles
**Learning:** When calculating values via linear interpolation (like percentiles), computing the exact index may result in a fractional weight of exactly zero (e.g. `p=0.5` with odd length). Continuing to calculate the adjacent element (`vHi = slices.Min(data[lo+1:])`) causes an unnecessary O(N) slice scan, heavily taxing the CPU in hot loops.
**Action:** Always implement early returns to skip expensive adjacent-element lookups (like O(N) scans) if the fractional weight is exactly zero, as their values will be entirely discarded by the interpolation formula anyway.
