...
func TestFuture_PanicIsolationWithVerticalWhitespace(t *testing.T) {
	g := New(DefaultConfig())
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	f, err := SubmitFunc(g, 10, func(c Context) (int, error) {
		panic("safe error message\rCRITICAL: You have been hacked\rgoroutine 1")
	})
	if err != nil {
		t.Fatalf("SubmitFunc failed: %v", err)
	}
	...
	if strings.Contains(getErr.Error(), "goroutine") {
		t.Errorf("Expected sanitized error without stack trace, got: %v", getErr)
	}
}
