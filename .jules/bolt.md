...
## 2024-06-02 - Early Return on Exact Percentiles
**Learning:** In `sortedPercentileMut` in `monitor.go`, finding the upper bound `vHi` for linear interpolation requires an O(N) `slices.Min` scan over the remaining slice. When the calculated index lands exactly on an element boundary (the fractional part is zero), this O(N) scan is completely wasted because its result is multiplied by zero.
**Action:** When calculating percentiles with linear interpolation on unsorted slices, always compute the fractional index difference (`frac`) first and return early if `frac == 0` to skip the unnecessary O(N) lookup for exact percentiles.
