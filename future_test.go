...
func TestFuture_PanicSpoofing(t *testing.T) {
	g := New(DefaultConfig())
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	f, err := SubmitFunc(g, 10, func(c Context) (int, error) {
		panic("fake payload\r\nmalicious spoofing\vmore spoofing")
	})
	if err != nil {
		t.Fatalf("SubmitFunc failed: %v", err)
	}
	...
	if strings.Contains(getErr.Error(), "malicious") || strings.Contains(getErr.Error(), "spoofing") {
		t.Errorf("Expected sanitized error without spoofed content, got: %v", getErr)
	}
}
