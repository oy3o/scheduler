...
func (c *closureTask[T]) Execute(ctx Context) (err error) {
	...
	defer func() {
		if p := recover(); p != nil {
			...
			// and log-spoofing attacks (terminal overwrite via carriage returns).
			// If the user's panic payload itself contains multiple lines (e.g. they
			// re-panicked an error with a stack trace) or malicious alternate whitespace,
			// we strip everything after the first occurrence to keep the API safe.
