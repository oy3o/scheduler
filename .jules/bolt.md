...
## 2025-05-24 - Early Return in Percentile Interpolation
**Learning:** When calculating values via linear interpolation (like percentiles), adjacent-element lookups (e.g., O(N) slice scans to find the next minimum value in a partially sorted array) are highly expensive. If the required index falls exactly on an integer boundary, the fractional weight is exactly zero.
**Action:** Implement early returns to skip expensive O(N) lookups if the fractional interpolation weight is exactly zero.
