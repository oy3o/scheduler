...
## 2025-02-13 - Panic Payload Log Spoofing
**Vulnerability:** Panic payload strings were only truncated at `\n`, allowing alternate vertical whitespace characters like `\r` and `\f` to bypass sanitization.
**Learning:** This could lead to stack trace leakage or terminal overwrite/log-spoofing attacks when the errors are logged.
**Prevention:** Always use `strings.IndexAny(pStr, "\n\r\f\v")` when sanitizing arbitrary text for single-line public error messages.
